...
char* check_errors(int instructions, struct store* all,char* all_opcodes[], int var_c, int lc, char* var_arr(), char* label_arr() ){
    //d&i stack
    int flg1=0;
    int i=0;
    ...
    for (i=0;i<instructions;i++){
        for(int k=0; k<10; k++){
                if(strcmp(label_arr(k), all[i].opc)==0){
                    if(all[i].opc[5] != ":"){
        ...
        for(int j=0;j<20;j++){
            ...
            //     if (strcmp(all[i].opc,all_opcodes[j])==0) {
            //         flg1=1;
            //     }
            //     else {
            //         return "Error: opcode incorrect/misspelled";
            //     }
            // }
            if (flg1==1 && ((strcmp(all[i].opc,"var")==0))){
                return "Error: Variables not declared in the beginning";
            }
            if (strcmp(all[i].opc,"hlt")==0){
                if(strcmp(all[instructions-1].opc,"hlt")==0){
                    return 0;
                }else{
                    return "error: Use hlt only at the end.";
                }
            } 
        }
...
int main() {
   ...
   char* inst1[20] = {"add", "sub", "mov", "mov", "ld", "st", "mul", "div", "rs", "ls", "xor", "or", "and", "not", "cmp", "jmp", "jlt", "jgt", "je", "hlt"};
    ...
    // printf("%s", check_errors(instructions,all,inst1));  //;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
